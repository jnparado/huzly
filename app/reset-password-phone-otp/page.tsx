...
function ResetPasswordPhoneOtpInner() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (!phone) throw new Error("Missing phone. Go back and enter your phone.");
      const token = code.trim();
      if (!token) throw new Error("Enter the code from the SMS.");
      ...
      setSuccess(true);
      window.setTimeout(() => router.push("/login/phone"), 900);
    } catch (err) {
      const message =
    ...
    <div className="mx-auto w-full max-w-sm px-6 py-10">
      <div className="mb-6 text-center">
        ...
        <div className="mt-2 text-sm text-zinc-500">
          ...
          <span className="font-medium text-zinc-700">{phone || "your phone"}</span>
        </div>
      </div>
...
export default function ResetPasswordOtpPhonePage() {
  return (
    <div className="min-h-full flex flex-col bg-zinc-50">
      <Suspense fallback={null}>
        <ResetPasswordPhoneOtpInner />
      </Suspense>
    </div>
  );
}
